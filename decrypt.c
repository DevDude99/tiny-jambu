...
// Function to write the decrypted contents to a file
...
int pad_key(const char* key_str, unsigned char* key) {
    ...
    // Copy the passphrase to the key, zero-padding if necessary
    for (int i = 0; i < key_len; i++) {
        key[i] = (unsigned char)key_str[i];
    }

    // Pad with zeros if the key is shorter than 24 bytes
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        fprintf(stderr, "Usage: %s <encrypted_file> <key> <output_file>\n", argv[0]);
        return 1;
    }
    ...
    unsigned char* c = read_file(encrypted_file, &clen);

    unsigned char* decrypted_text = (unsigned char*)malloc(clen - 8);  // Remove the 8-byte MAC
    if (!decrypted_text) {
        perror("Memory allocation error");
        return 1;
    }
    ...
    int decrypt_status = crypto_aead_decrypt(decrypted_text, &mlen, NULL, c, clen, NULL, 0, iv, key);

    if (decrypt_status != 0) {
        fprintf(stderr, "Decryption failed - check the password\n");
        return 1;
    }

    write_file(output_file, decrypted_text, mlen);
